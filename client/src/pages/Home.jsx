  ...
  return (
    <Link
      ...
      className="group relative bg-white rounded-2xl border border-gray-100 overflow-hidden 
                 transition-all duration-300 hover:-translate-y-2 hover:shadow-2xl hover:shadow-primary-500/10 flex flex-col"
    >
      ...
      <div className="relative h-48 overflow-hidden">
        ...
        {post.category && (
          <div className="absolute top-3 left-3">
            <span className="bg-white/90 backdrop-blur text-primary-700 text-xs font-bold px-3 py-1 rounded-full shadow">
              {post.category}
            </span>
          </div>
        )}
      ...
      <div className="p-5 flex flex-col flex-1">
        ...
        <div className="flex items-center justify-between mt-5 pt-3 border-t border-gray-100 text-xs text-gray-400">
          ...
          <div className="flex items-center space-x-2">
            <div className="h-7 w-7 rounded-full bg-gradient-to-br from-primary-500 to-violet-500 
                            flex items-center justify-center text-white text-[10px] font-bold">
              {(post.author?.name || 'L').charAt(0)}
            </div>
            <span className="font-medium text-gray-600">
              {post.author?.name || 'Library'}
            </span>
          </div>
      ...
      {/* Hover bottom accent */}
      <div className="absolute bottom-0 left-0 w-0 h-[3px] bg-gradient-to-r from-primary-500 to-violet-500 
...
export default function Home() {
  ...
  animate="show"
  variants={stagger}
  className="relative overflow-hidden bg-[#0f172a] text-white min-h-[600px] flex items-center"
>
  {/* Animated gradient blobs */}
{/* Background: image + gradient blobs */}
<div className="absolute inset-0 overflow-hidden pointer-events-none">

  {/* 🖼️ Light background image (optimized) */}
  <img
    src="https://images.unsplash.com/photo-1524995997946-a1c2e315a42f?q=80&w=1400"
  ...
  <div className="absolute -top-32 -left-32 h-[400px] w-[400px] rounded-full bg-primary-600/25 blur-[90px]" />
  <div className="absolute top-20 right-0 h-[320px] w-[320px] rounded-full bg-indigo-600/20 blur-[80px]" />
  <div className="absolute bottom-0 left-1/3 h-[250px] w-[250px] rounded-full bg-violet-600/20 blur-[70px]" />
  ...
  <div className="relative max-w-7xl mx-auto px-6 pt-12 pb-24 w-full">
    <div className="max-w-3xl mx-auto text-center">
      {/* Badge */}
      <motion.div variants={fadeUp} className="inline-flex items-center space-x-2 bg-white/10 backdrop-blur border border-white/20 px-4 py-2 rounded-full text-sm font-medium mb-4">
        <SparklesIcon className="h-4 w-4 text-yellow-400" />
        <span className="text-white/90 text-xs sm:text-sm">10,000+ books — grow your mind today</span>
      </motion.div>

      {/* Heading */}
      <motion.h1 variants={fadeUp} className="text-4xl sm:text-6xl lg:text-7xl font-black leading-[1.1] mb-2">
        Your library,
      ...
      {/* Subtitle */}
      <motion.p variants={fadeUp} className="text-base sm:text-lg text-white/70 mb-2 max-w-xl mx-auto leading-relaxed">
      ...
      {/* Search bar - mb-16 pushes the stats down, making search feel higher */}
      <motion.form 
        variants={fadeUp} 
        onSubmit={handleSearch}
        className="flex flex-col sm:flex-row gap-2 max-w-2xl mx-auto bg-white/10 backdrop-blur border border-white/20 p-2 rounded-2xl shadow-2xl mb-2"
      >
        ...
        <button type="submit"
          className="px-8 py-3 bg-primary-500 hover:bg-primary-400 text-white font-bold rounded-xl transition-all text-sm shadow-lg whitespace-nowrap active:scale-95"
        >
          Search
        </button>
      </motion.form>

      {/* Stats row - mt-0 or small mt ensures it doesn't crowd the search bar */}
      <motion.div variants={fadeUp} className="flex flex-wrap justify-center gap-8 sm:gap-12 mt-2">
        {[['10K+', 'Books'], ['5K+', 'Members'], ['500+', 'Authors'], ['50+', 'Categories']].map(([n, l]) => (
          <div key={l} className="text-center">
            <p className="text-2xl sm:text-3xl font-black text-white">{n}</p>
            <p className="text-[10px] sm:text-xs text-white/50 uppercase tracking-[0.2em] font-bold">{l}</p>
          </div>
        ))}
      </motion.div>
    </div>
...
<div className="max-w-7xl mx-auto px-4 -mt-5 relative z-10">
  <div className="w-full aspect-[6/1] flex items-center justify-center overflow-hidden rounded-2xl shadow-2xl bg-gray-50">
    <BannerSlot 
      placement="home_hero" 
      ...
      {cats.map(cat => (
        ...
        >
          <Link
            ...
            className="group flex flex-col items-center p-4 bg-white border border-gray-100 rounded-2xl shadow-sm hover:shadow-xl text-center overflow-hidden"
          >
            {/* Hover glow background */}
            <div className="absolute inset-0 bg-gradient-to-br from-primary-100 to-transparent opacity-0 group-hover:opacity-100 transition-opacity" />

            {/* Icon */}
            <span className="relative text-3xl mb-2 transition-transform duration-300 group-hover:scale-125 group-hover:-rotate-6">
              {cat.icon || '📚'}
            </span>

            {/* Name */}
            <span className="relative text-xs font-semibold text-gray-700 group-hover:text-primary-700 transition-colors">
              {cat.name}
            </span>
          </Link>
        </motion.div>
      ...
      <section className="relative py-16 bg-gradient-to-b from-white to-gray-50 border-y border-gray-100 overflow-hidden">
        <div className="max-w-7xl mx-auto px-4">
          ...
          <div className="flex items-center justify-between mb-8">
            <SectionHeader
              title="Popular Books"
              subtitle="Most borrowed and trending this month"
              link="/books"
            />
            ...
            <div className="hidden sm:flex space-x-2">
              <button
                ...
                className="p-2.5 rounded-xl bg-white shadow-sm border border-gray-200 hover:bg-primary-50 hover:text-primary-600 hover:shadow-md transition-all"
              >
                <ChevronLeftIcon className="h-5 w-5" />
              </button>
          ...
          <div className="relative">
            ...
            <div
              ref={booksRef}
              className="flex space-x-5 overflow-x-auto pb-4 scrollbar-hide snap-x scroll-smooth"
            >
              {books.map(book => (
                <div
                  key={book._id}
                  className="flex-shrink-0 w-44 snap-start transition-transform duration-300 hover:-translate-y-2"
                >
                  <BookCard
                    book={book}
                    onReserve={handleReserve}
                    priceDisplay={
                      book.isDigitalSale ? formatPrice(book.digitalPrice) : null
                    }
                  />
                </div>
              ))}
...
{/*      {featured.length > 0 && (
      ...
      {featured.length > 0 && (
          <section className="relative py-20 bg-gradient-to-br from-[#0f172a] to-[#1e1b4b] text-white overflow-hidden">

            {/* subtle background glow */}
            <div className="absolute inset-0 opacity-20 pointer-events-none">
              <div className="absolute top-10 left-10 w-72 h-72 bg-primary-500 blur-[120px] rounded-full" />
              <div className="absolute bottom-0 right-10 w-72 h-72 bg-violet-500 blur-[120px] rounded-full" />
            </div>
              ...
              >
                {featured.map(a => (
                  <Link
                    ...
                    className="group relative text-center p-4 rounded-2xl backdrop-blur bg-white/5 border border-white/10 hover:border-primary-400/40 transition-all duration-300 hover:-translate-y-2 hover:shadow-xl"
                  >
                    {/* hover glow */}
                    <div className="absolute inset-0 rounded-2xl bg-gradient-to-br from-primary-500/10 to-violet-500/10 opacity-0 group-hover:opacity-100 transition-opacity" />

                    {/* Avatar */}
                    <div className="relative mx-auto mb-3 h-24 w-24">
                      <div className="absolute inset-0 rounded-full bg-gradient-to-br from-primary-400 to-violet-500 opacity-0 group-hover:opacity-100 blur-md scale-110 transition" />

                      <img
                        src={getImageUrl(a.avatar)}
                        alt={a.name}
                        onError={imgOnError}
                        className="relative h-full w-full rounded-full object-cover object-top border-2 border-white/20 group-hover:border-primary-400 transition-all duration-300"
                      />
                    </div>

                    {/* Name */}
                    <p className="text-sm font-bold text-white group-hover:text-primary-300 transition-colors leading-tight">
                      {a.name}
                    </p>

                    {/* Nationality */}
                    {a.nationality && (
                      <p className="text-xs text-white/40 mt-1 group-hover:text-white/60 transition">
                        {a.nationality}
                      </p>
                    )}
                  </Link>
...
{/*      <section className="max-w-7xl mx-auto px-4 py-16">
      ...
      <section className="relative py-20 bg-gradient-to-b from-gray-50 to-white overflow-hidden">
        ...
        <div className="absolute inset-0 pointer-events-none opacity-30">
          <div className="absolute top-10 left-10 w-72 h-72 bg-primary-200 blur-[120px] rounded-full" />
          <div className="absolute bottom-0 right-10 w-72 h-72 bg-violet-200 blur-[120px] rounded-full" />
        </div>
        ...
        <div className="relative max-w-7xl mx-auto px-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            {(cms?.featuresSection?.length ? cms.featuresSection : [
              ...
              <div
                key={i}
                className="group relative bg-white border border-gray-100 rounded-3xl p-8 shadow-sm hover:shadow-2xl transition-all duration-300 hover:-translate-y-2 overflow-hidden"
              >
                ...
                <div className="relative">
                  ...
                  {/* Icon box */}
                  <div className="w-14 h-14 flex items-center justify-center rounded-2xl bg-primary-50 text-2xl mb-5 group-hover:scale-110 group-hover:bg-primary-100 transition-all">
                    {f.icon}
                  </div>
...
{testimonials.length > 0 && (
  <section className="relative bg-[#0f172a] py-24 overflow-hidden">
    ...
    <div className="absolute inset-0 pointer-events-none">
      <div className="absolute top-[-100px] left-[-100px] w-[300px] h-[300px] bg-primary-600/20 blur-[120px] rounded-full" />
      <div className="absolute bottom-[-100px] right-[-100px] w-[300px] h-[300px] bg-violet-600/20 blur-[120px] rounded-full" />
    </div>
    ...
    <div className="relative max-w-6xl mx-auto px-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {testimonials.map((t, i) => (
          <div
            key={i}
            className="group relative bg-white/5 border border-white/10 backdrop-blur rounded-2xl p-7 
                       transition-all duration-300 hover:-translate-y-2 hover:shadow-2xl hover:shadow-primary-500/10 overflow-hidden"
          >
            ...
            {/* Comment */}
            <p className="relative text-white/80 text-sm leading-relaxed italic mb-6">
              "{t.comment}"
            </p>

            {/* User */}
            <div className="relative flex items-center space-x-3">
              <div className="h-11 w-11 rounded-full bg-gradient-to-br from-primary-500 to-violet-500 
...
{/*      {!user && (
      ...
      {!user && (
        <section className="relative overflow-hidden py-24 text-center text-white">
          ...
          {/* Glow blobs */}
          <div className="absolute inset-0 pointer-events-none">
            <div className="absolute -top-24 -left-24 w-[300px] h-[300px] bg-white/10 blur-[120px] rounded-full" />
            <div className="absolute bottom-[-100px] right-[-100px] w-[300px] h-[300px] bg-violet-400/20 blur-[120px] rounded-full" />
          </div>
          ...
          <div className="relative max-w-2xl mx-auto px-4">
            ...
            <h2 className="text-4xl sm:text-5xl font-extrabold mb-5 leading-tight">
              {cms?.ctaTitle || "Join thousands of readers"}
            </h2>

            {/* Description */}
            <p className="text-primary-100 mb-10 text-lg leading-relaxed">
              {cms?.ctaDescription ||
                "Get unlimited access to our library — start free today."}
            </p>
            ...
            <div className="flex justify-center gap-4 flex-wrap">
              ...
              <Link
                to="/register"
                className="group relative px-8 py-4 bg-white text-primary-700 rounded-2xl font-black text-sm shadow-xl overflow-hidden transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl"
              >
              ...
              <Link
                to="/books"
                className="px-8 py-4 border border-white/30 text-white rounded-2xl font-semibold text-sm 
                           backdrop-blur hover:bg-white/10 transition-all duration-300 hover:-translate-y-1"
              >
                Browse Books
              </Link>
